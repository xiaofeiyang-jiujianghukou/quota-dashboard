...
export function evaluateQuotaReminders(quota) {
  try {
    ...
    const SHORT_RE = /(5\s*小时|滚动|rolling|会话窗口|session|five_hour|-5h)/i;
    const WEEK_RE = /(每周|周额度|weekly)/i;
    const MONTH_RE = /(每月|月度|月额度|monthly)/i;
    const FULLY_USED = 99.5; // "见底"= 彻底无额度（已用≈100%），不是低余量
    const usedPctOf = (it) => {
      if (it.percentUsed != null) return it.percentUsed;
      if (it.remainingPercent != null) return 100 - it.remainingPercent;
      if (it.total != null && it.remaining != null && it.total > 0) return 100 - (it.remaining / it.total) * 100;
      return null;
    };
    const fullyUsed = (it) => it.resetAt && (usedPctOf(it) ?? -1) >= FULLY_USED;

    for (const pv of quota.providers || []) {
      if (!pv.ok) continue;
      const its = pv.items || [];
      // 上层窗口（月 > 周）彻底用光(0/100%)时，下层即使还剩 1% 也无意义 → 只提醒见底的那一层恢复。
      const monthFullyUsed = its.find((it) => MONTH_RE.test(`${it.title} ${it.key}`) && fullyUsed(it));
      const weekFullyUsed = !monthFullyUsed ? its.find((it) => WEEK_RE.test(`${it.title} ${it.key}`) && fullyUsed(it)) : null;
      const bindingWindow = monthFullyUsed || weekFullyUsed;
      // 目标条目：有见底的上层窗口 → 只它；否则 → 用过的短窗口（5小时/会话/滚动）按自己恢复提醒
      const scope = bindingWindow
        ? [bindingWindow]
        : its.filter(
            (it) =>
              it.resetAt &&
              SHORT_RE.test(`${it.title} ${it.key} ${it.resetLabel || ''}`) &&
              (usedPctOf(it) ?? -1) >= 0.1
          );
      for (const it of scope) {
        const t = new Date(it.resetAt).getTime();
        if (!Number.isFinite(t)) continue;
        // 去重按「平台 + 恢复时刻」整周期计（同平台多模型/行同刻恢复只提醒一次）
        const base = `${pv.id}|${t}`;
