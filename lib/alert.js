...
export function evaluateQuotaReminders(quota) {
  try {
    ...
    const NEAR_DEPLETED = 90; // "见底"阈值：已用 ≥90% 即视为上级额度是当前瓶颈
    const usedPctOf = (it) => {
    ...
    const nearDepleted = (it) => it.resetAt && (usedPctOf(it) ?? -1) >= NEAR_DEPLETED;

    for (const pv of quota.providers || []) {
      if (!pv.ok) continue;
      const its = pv.items || [];
      // 上级窗口（月 > 周）见底时，其下所有短窗口提醒无意义 → 只盯该上级窗口的恢复。
      // 例：方舟月额度剩4%（周还有）→ 只提醒每月；智谱周额度耗尽 → 只提醒每周。
      const monthNear = its.find((it) => MONTH_RE.test(`${it.title} ${it.key}`) && nearDepleted(it));
      const weekNear = !monthNear ? its.find((it) => WEEK_RE.test(`${it.title} ${it.key}`) && nearDepleted(it)) : null;
      const seniorNear = monthNear || weekNear;
      // 目标条目：有见底的上级窗口 → 只它；否则 → 用过的短窗口（5小时/会话/滚动）
      const scope = seniorNear
        ? [seniorNear]
        : its.filter(
            (it) =>
              it.resetAt &&
