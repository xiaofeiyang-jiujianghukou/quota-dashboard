  ...
  try {
    ...
    for (const pv of quota.providers || []) {
      ...
      for (const it of pv.items || []) {
        ...
        // 提醒范围：当前只盯「短滚动窗口」——即 智谱 Coding Plan · 5 小时滚动额度 这类
        // 会频繁耗尽/恢复的额度（周/月大额度平时用不完，不打扰）。去掉该过滤即恢复对所有窗口提醒。
        const isShortRolling = /(5\s*小时|滚动|rolling)/i.test(`${it.title} ${it.key}`);
        if (!isShortRolling) continue;
        const t = new Date(it.resetAt).getTime();
        if (!Number.isFinite(t)) continue;
        const base = `${pv.id}|${it.key}|${t}`;
