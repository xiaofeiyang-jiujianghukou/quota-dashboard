...
export function evaluateQuotaReminders(quota) {
  try {
    ...
    const WEEK_RE = /(每周|周额度|weekly|周)/i;
    const usedPctOf = (it) => {
    ...
    for (const pv of quota.providers || []) {
      if (!pv.ok) continue;
      const its = pv.items || [];
      // 周额度耗尽时（如智谱每周 100%）：5h 窗口再提醒无意义 → 只盯该周窗口的恢复
      const weeklyExhausted = its.find(
        (it) => it.resetAt && WEEK_RE.test(`${it.title} ${it.key}`) && (usedPctOf(it) ?? -1) >= 99
      );
      // 目标条目：周耗尽 → 只有那条周窗口；否则 → 用过的短窗口（5小时/会话/滚动）
      const scope = weeklyExhausted
        ? [weeklyExhausted]
        : its.filter(
            (it) =>
              it.resetAt &&
