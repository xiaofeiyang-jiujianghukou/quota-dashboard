      ...
      for (const it of pv.items || []) {
        ...
        if (rp != null && thr != null && rp < thr) {
          ...
          if (shouldSend(key)) {
            ...
            msgs.push(`- **${pv.name} · ${it.title}**：余量仅剩 ${rp.toFixed(0)}%（阈值 ${thr}%）`);
          }
        }
        // 2) 到期提醒（按 平台+日期 去重）
        if (it.expiresAt) {
          ...
          if (!Number.isNaN(t)) {
            ...
            if (limit != null && days <= limit) {
              ...
              if (shouldSend(key)) {
                ...
                msgs.push(`- **${pv.name} · ${it.title}**：${txt}（${new Date(t).toISOString().slice(0, 10)}）`);
              }
            }
          }
        }
        ...
        if (it.kind === 'balance' && it.remaining != null) {
          ...
          if (bal != null && it.remaining < bal) {
            ...
            if (shouldSend(key)) {
              sent[key] = new Date().toISOString();
              msgs.push(`- **${pv.name}**：余额仅 ¥${it.remaining}（阈值 ¥${bal}）`);
            }
          }
        }
        ...
        if (it.extra && it.extra.monthCost != null && it.extra.budgetMonthly > 0) {
          ...
          if (pctUsed >= 80) {
            ...
            if (shouldSend(key)) {
              ...
              msgs.push(`- **${pv.name}**：本月消费 ¥${used.toFixed(2)}，${level}月预算 ¥${budget} 的 ${pctUsed.toFixed(0)}%`);
            }
          }
        }
      }
