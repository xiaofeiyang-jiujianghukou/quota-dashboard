...
export const AUTH_SCHEMA = {
  ...
  deepseek: {
    name: 'DeepSeek',
    method: 'both',
    note: 'API Key 查余额；控制台会话（authorization token + cookie）查当天/当周/当月消费与环比',
    fields: [
      ['apiKey', 'API Key', 'key'],
  ...
  try {
    ...
    if (pid === 'ark') {
      ...
      if (p.sessionToken && p.sessionCookie) {
        ...
        const url = `https://platform.deepseek.com/api/v0/usage/by_api_key/cost?start=${nowSec - 86400}&end=${nowSec + 86400}&tz=28800`;
        const res = await fetch(url, {
          headers: {
            ...
            'user-agent': 'Mozilla/5.0 (Linux; Android 15) AppleWebKit/537.36 Chrome/151.0.0.0 Mobile Safari/537.36',
