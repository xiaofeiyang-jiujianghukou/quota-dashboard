...
const DEFAULT_CONFIG = {
  port: 8899,
  authToken: '', // 写接口鉴权 token（上云时设置，扩展同步会话需携带；留空=不鉴权，仅本机使用）
  refreshIntervalSec: 60,
  requestTimeoutMs: 30000,
  providers: {
    ark: { enabled: true, accessKeyId: '', secretKey: '', region: 'cn-beijing', planTier: '', expiresAt: '', sessionCookie: '', csrfToken: '', webId: '' },
    ark2: { enabled: false, accessKeyId: '', secretKey: '', region: 'cn-beijing', planTier: '', expiresAt: '', sessionCookie: '', csrfToken: '', webId: '' },
    ...
    deepseek: { enabled: true, apiKey: '', sessionToken: '', sessionCookie: '', budgetMonthly: 0, expiresAt: '' },
  },
  pricing: {}, // 套餐价格覆盖：{ ark: { lite: '¥xx/月' }, ... }，留空用内置默认价
  alert: {
    enabled: true,
    wecom: { enabled: true, chatName: '' }, // chatName 留空 = 发给授权人自己
    rules: { remainingPercentBelow: 20, expiresWithinDays: 3, balanceBelow: 10, peakNotify: true },
    dailyDigest: true,
  },
};
...
export function loadConfig() {
  ...
  if (env.QUOTA_AUTH_TOKEN) cfg.authToken = env.QUOTA_AUTH_TOKEN;
  ...
  if (env.DEEPSEEK_SESSION_TOKEN) cfg.providers.deepseek.sessionToken = env.DEEPSEEK_SESSION_TOKEN;
  if (env.DEEPSEEK_SESSION_COOKIE) cfg.providers.deepseek.sessionCookie = env.DEEPSEEK_SESSION_COOKIE;
  if (env.DEEPSEEK_BUDGET_MONTHLY) cfg.providers.deepseek.budgetMonthly = num(env.DEEPSEEK_BUDGET_MONTHLY, cfg.providers.deepseek.budgetMonthly);
