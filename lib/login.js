...
/**
 * 自动启动一个带调试端口的 Chrome 副本（无需用户跑脚本）：
 * 复制日常 Chrome 的登录 cookie 到独立数据目录，用 --user-data-dir + 调试端口启动。
 * 这样绕开「默认数据目录禁止调试端口」的限制，cookie 由系统钥匙串解密后即可 CDP 读明文。
 */
async function launchDebugChrome(port) {
  const home = os.homedir();
  const src = home + '/.config/google-chrome';
  const dst = process.env.CHROME_DEBUG_PROFILE || home + '/.cache/quota-dashboard/chrome-debug';
...
async function cdpLogin(provider) {
  ...
  try {
    ...
    // 调试端口未运行 → 自动用副本启动调试 Chrome，再重试连接
    ...
    if (!connected) {
      throw new Error('自动启动调试 Chrome 失败。请确认本机已安装 Chrome，且看板（node server.js）与浏览器在同一台机器上运行。');
    }
  }
